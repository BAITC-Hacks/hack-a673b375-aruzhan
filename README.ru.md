...
| --- | --- |
| Личный карьерный маршрут | Постоянно видимая карьерная цель, карта и карточка выбранного шага в двух колонках на компьютере, до трёх доступных шагов A/B/C и метки длительности, доступности и навыков. |
| Объяснимые рекомендации | Результат учитывает должность, грейд, предварительные требования, историю участия, доступность сессий и прирост целевых навыков. Исходные записи, условия допуска и диагностика фильтров доступны в свёрнутых блоках. |
...
2. **Объясните первую рекомендацию.** Выберите A и прочитайте `Why this step` и сравните масштабированные полосы `Current estimate` → `After learning` → `Target` в блоке `Expected change`. Откройте `Why it fits you & what comes next` → `View supporting records`, чтобы увидеть исходные данные, затем нажмите `Preview impact`. Расчётное покрытие требований меняется с **42% на 48%**, а покрытие по оценённым навыкам остаётся **42%**. Нажмите `Undo` и попробуйте `2D view`, чтобы показать альтернативное управление.
...
Дополнительно, если провайдер настроен: раскройте `AI career coach`, нажмите `Explain my next steps` и изучите `What the coach checked`. Выберите предложенный шаг: в `Why this step` появится объяснение, а в `Practice challenge` — конкретное задание. Начните с `Build`, затем раскройте `Deliver & check`, чтобы увидеть ожидаемый результат и критерии. Задание включает развиваемый навык, небольшую задачу, итоговый артефакт и два-три проверяемых критерия успеха. Используйте вымышленные примеры или локальную песочницу. Эти необязательные AI-задания находятся вне каталога мероприятий: они не записывают на обучение, не отмечают прохождение и не начисляют подтверждённый рост навыков. Без ключа коуч сообщает о недоступности.
...
Обычные модули HTML/CSS/JavaScript, Three.js, встроенные HTTP-сервер и тестовый раннер Node. Браузер и backend используют один неизменённый движок рекомендаций. Manrope раздаётся локально через закреплённый `@fontsource-variable/manrope` `5.3.0`; браузер не обращается к стороннему сервису шрифтов.

Светлый интерфейс использует белые карточки, почти чёрный текст `#17211D`, вторичный текст `#526159` и зелёные/жёлтые акценты Halyk. Иерархия на компьютере: заголовок страницы 40px/800, заголовок карточки 24px/700, основной текст 17px/500, подписи 14px/600. Расчётные навыки и возможные изменения после обучения визуально отделены от покрытия по оценённым навыкам.
