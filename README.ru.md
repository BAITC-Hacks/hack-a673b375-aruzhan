...
Дополнительно, если провайдер настроен: раскройте `AI career coach`, нажмите `Explain my next steps` и изучите `What the coach checked`. Выберите предложенный шаг: AI-объяснение появится в `Why this helps`, а практическое предложение — в `Try it at work`. Такие упражнения находятся вне каталога мероприятий и не создают запись на обучение или подтверждённый рост навыков. Без ключа коуч сообщает о недоступности.
...
Коуч использует `retrieve_profile`, `inspect_gaps` и `find_eligible_activities`. Он выбирает ID мероприятий и навыков, коды причин, а также формирует краткие объяснения обычным текстом и практические предложения со ссылками на исходные записи. Код проверяет допуск, принадлежность ссылок к соответствующим данным, формат текста и отдельные виды необоснованных обещаний; эти проверки не доказывают смысловую точность AI-текста. Точные приросты, даты и прогресс рассчитываются детерминированно. Исходные записи раскрываются по желанию; журнал работы инструментов остаётся доступен.

Ограничения выполнения: **30 секунд, 5 раундов модели, 9 вызовов инструментов, 1 200 выходных токенов за раунд и 24 000 токенов суммарно**. Ключи остаются на сервере. Сервер слушает `127.0.0.1` и отдаёт только явно разрешённые файлы frontend, общий чистый модуль расчёта и синтетические данные. Остальной код backend, тесты, скрипты и `.env` исключены из публичных маршрутов.

**Одна ограниченная проверка реального провайдера прошла успешно** **2026-09-23** для `E0101` с `gpt-6-luna`: три проверенных шага, три вызова инструментов, **8 771 мс**, **4 907 входных токенов** и **458 выходных токенов**. Это подтверждает интеграцию на одном профиле, но не доказывает точность рекомендаций или общее качество планов. Офлайн-тесты дополнительно проверяют подготовленные ответы и сценарии ошибок.
...
Проверено локально **2026-09-23**: **59 из 59 тестов прошли**; `npm run evaluate`, `npm run check` и `git diff --check` выполнены успешно. Проверки охватывают локальную реализацию и подготовленные ответы провайдера. Отдельная проверка реального API выше охватывает один профиль; широкая оценка релевантности AI и результатов пока не проводилась.
