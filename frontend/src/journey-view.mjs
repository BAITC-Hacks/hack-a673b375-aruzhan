...
export function stepCard(item, result, preview, skills, asOfDate, coachStep = null) {
  ...
  const scaleMax=Math.max(5,...item.improvements.flatMap(gap=>[gap.current,gap.afterEvent,gap.requirement]));
  const scalePercent=level=>Math.round(level/scaleMax*10000)/100;
  const gains=item.improvements.map(gap=>{
    const skillControl=Object.hasOwn(target.required_skills,gap.skill_id)
      ? `<button type="button" data-skill-focus="${e(gap.skill_id)}">${e(gap.name)}</button>`
      : `<b>${e(gap.name)}</b>`;
    return `<div class="step-gain ${preview ? "preview-change" : ""}" data-skill-change="${e(gap.skill_id)}">
      <div class="skill-change-heading"><div>${skillControl}${gap.critical ? '<small class="critical-label">Critical for your goal</small>' : ""}</div><div class="skill-change-values" aria-label="Current estimate ${gap.current}; after learning ${gap.afterEvent}; ${isPrerequisite ? "prerequisite" : "target"} ${gap.requirement}"><span>${gap.current}</span><span class="gain-arrow" aria-hidden="true">→</span><strong>${gap.afterEvent}</strong></div></div>
      <div class="skill-change-track" aria-hidden="true"><span class="skill-change-after" style="width:${scalePercent(gap.afterEvent)}%"></span><span class="skill-change-current" style="width:${scalePercent(gap.current)}%"></span><span class="skill-change-target" style="left:${scalePercent(gap.requirement)}%"></span></div>
      <div class="skill-change-labels"><span>Current estimate <b>${gap.current}</b></span><span>After learning <b>${gap.afterEvent}</b></span><span>${isPrerequisite ? "Prerequisite" : "Target"}: <b>${gap.requirement}</b></span></div>
    </div>`;
  }).join("");
  ...
  return `<article class="step-card" data-event="${e(item.event.event_id)}">
    <header class="step-card-header"><h3>${e(item.event.title)}</h3><div class="step-duration"><strong>${item.event.duration_hours} hours</strong><small>${e(typeLabel(item.event))}</small></div></header>
    <div class="step-chips" aria-label="Skills and availability">${item.improvements.map(gap=>`<span class="step-chip skill-chip">${e(gap.name)}</span>`).join("")}<span class="step-chip availability-chip">${item.nextSession ? "Next session · " : "Self-paced · "}${e(shortSession(item))}</span></div>
    <section class="step-reason"><h4>Why this step${narrative ? '<span class="ai-label">AI explanation</span>' : ""}</h4><p class="step-why">${narrative ? e(narrative.whyThisStep) : why}</p></section>
    <section class="expected-change" aria-label="Expected skill changes"><div class="change-heading"><h4>Expected change</h4><small>Scale 0–${scaleMax} · estimated levels</small></div><div class="step-gains">${gains}</div><p class="estimate-note">Learning estimates. Your assessed skills stay unchanged.</p></section>
    <p class="prerequisite-line">${prerequisites ? `Starting levels met (learning estimate): ${e(prerequisites)}.` : "No prior skill levels required."}</p>
    <div class="step-actions">${impact || `<button type="button" class="primary-button" data-preview="${e(item.event.event_id)}"><span aria-hidden="true">◈</span> Preview impact</button><p class="preview-explainer">See what would change. Nothing is saved or booked.</p>`}</div>
    ${practice ? `<section class="application-tip" aria-label="Practice challenge"><h4>Practice challenge <span class="ai-label">${e(item.improvements.find(gap=>gap.skill_id===practice.skillId).name)}</span></h4><div class="practice-build"><span class="practice-phase">Build</span><p>${e(practice.task)}</p></div><details class="practice-details"><summary>Deliver &amp; check</summary><div class="practice-deliver"><h5>Deliver</h5><p>${e(practice.deliverable)}</p></div><div class="practice-checks"><h5>Check</h5><ul>${practice.successCriteria.map(check=>`<li>${e(check)}</li>`).join("")}</ul></div></details><small>AI suggestion · sandbox task outside the activity catalog. Completing it does not update assessed skills.</small></section>` : ""}
    <details class="evidence-details"><summary>Why it fits you & what comes next</summary><div class="human-explanation"><h4>You can take this step</h4><p>It is open to your current role and grade. ${item.event.event_id === "EV_036" ? "This speaking club welcomes repeat participation." : "You have not completed it before."} You have no unfinished participation in this activity.</p><p>${item.nextSession ? `Session listed in the ${e(formatDate(asOfDate))} catalog; booking is not confirmed.` : "Self-paced activity; no session booking is needed."}</p><h4>After this step</h4><p>${e(afterStep)}</p><p>These are expected learning gains. Your assessed skills stay unchanged until a new assessment.</p>${supportingRecords(evidence)}</div></details>
  </article>`;
}
