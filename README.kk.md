...
| --- | --- |
| Жеке мансап жолы | Three.js негізіндегі мансап картасы, қатысуға болатын үшке дейін A/B/C қадамы және әр іс-шараның жеке сипаттамасы. |
| Түсіндірілетін ұсыныстар | Нәтиже лауазымға, грейдке, алғышарттарға, қатысу тарихына, сессиялардың қолжетімділігіне және мақсатты дағдылардың өсуіне негізделеді. Бастапқы жазбалар мен сүзу диагностикасы ашық көрсетіледі. |
...
2. **Бірінші ұсыныстың себебін көрсетіңіз.** A қадамын таңдап, `Why this helps` түсіндірмесін оқыңыз. Бастапқы деректерді көру үшін `Why it fits you & what comes next` → `View supporting records` ашып, содан кейін `Preview impact` басыңыз. Талаптардың болжамды орындалуы **42%-дан 48%-ға** өседі, ал бағалау деректеріне негізделген көрсеткіш **42%** болып қалады. `Undo` басып, балама басқаруды көрсету үшін `2D view` режимін қосыңыз.
...
Провайдер бапталған болса, қосымша `AI career coach` бөлімін ашып, `Explain my next steps` таңдаңыз және `What the coach checked` жазбаларын қараңыз. Ұсынылған қадамды таңдағанда AI түсіндірмесі `Why this helps`, ал тәжірибелік ұсыныс `Try it at work` бөлімінде көрінеді. Мұндай жаттығулар іс-шаралар каталогынан тыс ұсынылады; олар оқуға тіркемейді және расталған дағды өсімін бермейді. Кілт болмаса, кеңесші қолжетімсіз екенін хабарлайды.
...
Кеңесші `retrieve_profile`, `inspect_gaps` және `find_eligible_activities` құралдарын пайдаланады. Ол іс-шара мен дағды ID-лерін, себеп кодтарын таңдап, дереккөздерге сілтемелері бар қарапайым мәтіндегі қысқа түсіндірмелер мен тәжірибелік ұсыныстар береді. Код қатысу талаптарын, сілтемелердің тиісті деректерге жатуын, мәтін пішімін және негізсіз уәделердің жекелеген түрлерін тексереді; бұл тексерулер AI мәтінінің мағыналық дұрыстығын дәлелдемейді. Нақты өсім, күндер мен прогресс ережелер бойынша есептеледі. Бастапқы жазбалар қалауыңыз бойынша ашылады; құралдарды пайдалану журналы қолжетімді.

Орындалу шектері: **30 секунд, модельдің 5 раунды, құралдарды 9 рет шақыру, әр раундта 1 200 шығыс токені және жиынтық 24 000 токен**. Құпия деректер серверде сақталады. Сервер `127.0.0.1` мекенжайында жұмыс істейді және тек рұқсат етілген frontend файлдарын, ортақ есептеу модулін және синтетикалық деректерді береді. Backend-тің өзге коды, тесттер, скрипттер және `.env` файлдары ашық маршруттарға кірмейді.

**Нақты провайдердің бір шектеулі тексеруі сәтті өтті:** **2026-09-23** күні `E0101` профилі үшін `gpt-6-luna` үш тексерілген қадам беріп, құралдарды үш рет шақырды; **8 771 мс**, **4 907 кіріс токені** және **458 шығыс токені** жұмсалды. Бұл бір профильдегі интеграцияны растайды, бірақ ұсыныстардың дәлдігін немесе барлық жоспарлардың сапасын дәлелдемейді. Офлайн тесттер дайын жауаптар мен қате жағдайларын қосымша тексереді.

## Технологиялар және репозиторий құрылымы

Қарапайым HTML/CSS/JavaScript модульдері, Three.js, Node-тың HTTP сервері мен тест іске қосқышы. Браузер мен backend бір ұсыныс модулін пайдаланады.
...
**2026-09-23** күні жергілікті тексерілді: **59 тесттің 59-ы өтті**; `npm run evaluate`, `npm run check` және `git diff --check` сәтті аяқталды. Бұл тексерулер жергілікті код пен провайдердің дайын жауаптарын қамтиды. Жоғарыдағы бөлек нақты API тексеруі бір профильді қамтиды; AI ұсыныстарының өзектілігі мен нәтижелері кең көлемде әлі бағаланған жоқ.
